...
test('reader channels never replace the input channel and all channels close with the peer', (t) => {
  ...
  t.after(() => {
    ...
    global.RTCPeerConnection = previousPeer;
  });
  ...
  const images = channel('ImageTransfer');
  rtc.peerConnection.ondatachannel({ channel: images });
  assert.equal(rtc.imageChannel, images);
  assert.equal(rtc.cbDataChannel, input);
  assert.equal(rtc.cbReaderChannel, reader);
  assert.deepEqual(
    rtc.peerConnection.channels.map((item) => item.label),
    ['MessageChannel', 'SessionReader']
  );
  ...
  images.close();
  images.onclose();
  assert.equal(rtc.imageChannel, null);
  rtc.openImageChannel();
  const replacement = rtc.imageChannel;
  assert.notEqual(replacement, images);
  assert.equal(replacement.readyState, 'open');
  // A late close callback from the old channel must not clear its replacement.
  images.onclose();
  assert.equal(rtc.imageChannel, replacement);
  const duplicate = channel('ImageTransfer');
  rtc.bindImageChannel(duplicate);
  ...
  assert.equal(images.readyState, 'closed');
  assert.equal(replacement.readyState, 'closed');
