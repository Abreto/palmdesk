...
- Pan mode combines local swiping with immediate remote tap selection: move to the sidebar, select a task, and move back to the content without toggling watch-only mode. Swipes and long presses do not select tasks. Watch-only mode remains read-only; use scroll mode to scroll a remote list.
- Compose text locally, including Chinese text, then send it to the host. Send Enter, common keys, and hardware keyboard input.
- Codex windows offer task and content views: enlarge the left sidebar, adjust its width, and retain separate zoom, pan, and keyboard settings while the window remains connected. Task navigation supports swipe-to-scroll and immediate tap selection while preserving input drafts. Touchscreen controllers start in scroll mode.
- Touch mode buttons, the watch-only toggle, and the zoom selector stay visible in a single toolbar row for direct access to scrolling, panning, and viewing without sending input. The watch-only button is highlighted when enabled. Display settings contain task/content views, sidebar width, keyboard visibility, and fullscreen. One arrow-key button opens a cross-shaped pad for repeated taps without reducing the viewing area. Tap outside, tap the toggle again, or press Escape to close it.
- This first version shows the original sidebar, where the bell can be tapped to show recent tasks. It does not automatically trigger the bell, read task states, or recreate the session list. Keep the sidebar expanded in the original app.
