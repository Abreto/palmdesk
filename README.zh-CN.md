...
macOS 上使用 `pnpm dist:mac`、Windows 上使用 `pnpm dist:win` 生成本地安装包。手动触发 **Desktop Prerelease** 工作流可构建 Apple Silicon、Intel Mac 和 Windows x64 安装包，全部成功后统一发布到 GitHub Prerelease，并附 SHA256 校验文件。这些测试包尚未完成发行签名或公证。版本规则、操作步骤及安装限制见 [桌面安装包与预发布](docs/DESKTOP_RELEASES.md)。
