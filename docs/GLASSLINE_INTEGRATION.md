...
6. 切回阅读保留位置、会话和连接；窗口输入草稿也保留。阅读时暂停视频传输，阻止窗口控制，并释放按住的键和鼠标按钮。
7. 返回后台挂起的页面时自动检查连接，必要时重新认证并建立 WebRTC。阅读组件在连接切换期间保留内容和滚动位置，重新检查读取授权；关闭读取仍会清除内容。窗口恢复引用由桌面保存并绑定控制端设备，断线后有效五分钟，恢复时重新核对原生窗口身份。不会重放文字输入或图片粘贴。
